...
exports.config = {
  ...
  before: async function (
    ...
    _specs: WebdriverIO.Config['specs'],
  ) {
    const emulator = getDeviceFromCapabilities('mobile');
    const isAndroid = emulator.isAndroid;
    ...
    const appIdentifier = isAndroid ? PACKAGE_NAME : BUNDLE_ID;

    const appId = isAndroid ? appIdentifier : undefined;
    const bundleId = isAndroid ? undefined : appIdentifier;

    const appPayload = {
      [isAndroid ? 'appId' : 'bundleId']: appIdentifier,
    };

    const isAppInstalled = await emulator.isAppInstalled(appId, bundleId);

    console.info('APP INSTALLED: ', isAppInstalled);

    const appState = await emulator.queryAppState(appId, bundleId);
    ...
    await emulator.execute(
      isAndroid ? 'mobile: activateApp' : 'mobile: launchApp',
      appPayload,
    );
   ...
   */
  afterTest: async function (_test: object, _context: object, result: any) {
    if (result.error || !result.passed) {
      const myEmulator = getDeviceFromCapabilities('mobile');
      const myBrowser = getDeviceFromCapabilities('browser');
      await myEmulator.takeScreenshot();
      await myBrowser.takeScreenshot();
    }
  },
